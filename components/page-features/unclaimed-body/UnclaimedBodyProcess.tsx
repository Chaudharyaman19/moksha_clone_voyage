...
export default function UnclaimedBodyProcess() {
  return (
    <section id="how-unclaimed-works" className="bg-[#fbf8f2] px-5 py-10">
      <div className="mx-auto max-w-[1500px] text-center">
        <p className="text-[16px] font-bold uppercase tracking-[0.12em] text-[#b27b22]">The Sewa Process</p>
        <h2 className="mt-3 text-[44px] font-medium text-[#064335] sm:text-[54px]" style={{ fontFamily: "Georgia, 'Times New Roman', serif" }}>
          How Unclaimed Body Sewa Works
        </h2>
        <div className="mt-3 flex items-center justify-center gap-3 text-[#bd8a35]">
        ...
        <div className="mt-10 grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4">
          {steps.map(([no, icon, title, text], index) => (
            <div key={no} className="relative px-5">
              ...
              <div className="relative mx-auto grid h-[94px] w-[94px] place-items-center rounded-full border border-[#d4c3a2] bg-[#f5f4ed] text-[#0c4c3b]">
              ...
              <h3 className="mt-4 text-[20px] font-bold text-[#0a4a39]">{title}</h3>
              <span className="mx-auto mt-3 block h-[2px] w-10 bg-[#c28c34]" />
              <p className="mt-4 text-[16px] font-medium leading-[1.5] text-[#4b5051]">{text}</p>
            </div>
          ))}
        </div>

        <div className="mt-9 grid grid-cols-1 gap-4 rounded-[12px] bg-[#f1f1ea] px-6 py-4 text-left md:grid-cols-2">
        ...
        <a href="#request-unclaimed-help" className="mx-auto mt-6 inline-flex h-[54px] min-w-[300px] items-center justify-between rounded-[7px] bg-[#004b39] px-7 text-[16px] font-bold uppercase text-white">
          Request Sewa Help
          <UBSIcon name="ArrowRight" className="h-6 w-6 text-[#d5a33a]" />
        </a>
      </div>
      <div className="mx-auto mt-8 h-[5px] max-w-[1500px] bg-[#004735]" />
    </section>
  );
}
