...
export default function OrganisationBehindMission() {
  const section = useWebsiteSection("unclaimed-organisation");
  const items = section?.items?.length ? section.items : defaultPillars.map(p => ({ title: p[1], description: p[2] }));
  const icons: UBSIconName[] = ["ShieldCheck", "People", "HeartHands", "Lotus"];

  // Fallbacks for the title formatting logic
  const defaultTitle = "The Organisation Behind Moksha Sewa";
      ...
      <div className="relative mx-auto max-w-7xl">
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-[31%_69%] lg:gap-5">
          ...
          <div className="pt-1 text-left lg:pt-2">
            ...
            <p className="mt-2 max-w-[320px] text-left text-[14px] font-bold uppercase leading-snug tracking-[0.06em] text-[#b27a21] sm:max-w-none sm:text-[18px]">{textOrFallback(section?.eyebrow, "Namo Gange Trust", 60)}</p>
            <h2 className="mt-3 text-left text-[30px] font-medium leading-tight text-[#064335] sm:text-[40px] lg:text-[50px] lg:leading-[1.05]" style={{ fontFamily: "Georgia, 'Times New Roman', serif" }}>
              {textOrFallback(title, defaultTitle, 150)}
            </h2>
            <p className="mt-3 max-w-[800px] text-left text-[16px] font-medium leading-7 text-[#404647] lg:text-[17px] lg:leading-[1.4]">
              {textOrFallback(section?.description, "A Humanitarian Initiative of Namo Gange Trust.", 600)}
            </p>
          </div>
        </div>
        ...
        <div className="relative mt-5 rounded-[14px] bg-[#004b39] px-4 pb-4 pt-10 text-white sm:mt-6 sm:px-5">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-0 xl:grid-cols-4">
            {items.map((item, index) => {
              ...
              return (
                <div key={item.title} className={`rounded-[10px] bg-white/5 px-4 py-4 text-center sm:rounded-none sm:bg-transparent sm:px-5 sm:py-3 lg:px-6 ${index ? "xl:border-l xl:border-[#8c895f]" : ""}`}>
                  ...
                  <h3 className="mt-2 text-[19px] font-semibold sm:text-[20px]" style={{ fontFamily: "Georgia, 'Times New Roman', serif" }}>{textOrFallback(item.title, "", 60)}</h3>
                  <span className="mx-auto mt-2 block h-[2px] w-9 bg-[#c99734]" />
                  <p className="mx-auto mt-2 max-w-[260px] text-[16px] leading-6 text-[#eeeae0] sm:leading-[1.35]">{textOrFallback(item.description, "", 160)}</p>
                </div>
              );
            })}
          </div>
