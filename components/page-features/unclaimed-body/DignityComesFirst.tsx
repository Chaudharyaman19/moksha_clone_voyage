...
export default function DignityComesFirst() {
  const section = useWebsiteSection("unclaimed-dignity-first");
  const items = section?.items?.length ? section.items : defaultPrinciples.map(p => ({ title: p[1], description: p[2] }));
  const icons: UBSIconName[] = ["ShieldCheck", "Lock", "Clipboard", "HeartHands"];
  ...
  return (
    <section className="relative overflow-hidden bg-[#003d31] px-5 py-6 text-white">
      ...
      <div className="relative mx-auto max-w-7xl text-center">
        ...
        <p className="mt-1 text-[16px] font-bold uppercase tracking-[0.12em] text-[#d0a247]">{textOrFallback(section?.eyebrow, "Sewa With Responsibility", 60)}</p>
        <h2 className="mt-1.5 text-[42px] font-medium sm:text-[50px]" style={{ fontFamily: "Georgia, 'Times New Roman', serif" }}>
          {textOrFallback(section?.title, "Dignity Comes First.", 150)}
        </h2>
        <p className="mx-auto mt-2 max-w-[760px] text-[16px] leading-[1.4] text-[#ecebe4]">
          {textOrFallback(section?.description, "Humanitarian support during a final journey requires sensitivity, privacy and responsible processes.", 600)}
        </p>
        ...
        <div className="mt-5 grid grid-cols-1 gap-0 sm:grid-cols-2 xl:grid-cols-4">
          {items.map((item, index) => {
            ...
            return (
              <div key={item.title} className={`px-6 py-2 ${index ? "xl:border-l xl:border-[#9a8f5f]" : ""}`}>
                ...
                <h3 className="mt-2 text-[20px] font-semibold" style={{ fontFamily: "Georgia, 'Times New Roman', serif" }}>{textOrFallback(item.title, "", 60)}</h3>
                <span className="mx-auto mt-2 block h-[2px] w-10 bg-[#be8c31]" />
                <p className="mt-2 text-[16px] leading-[1.35] text-[#e6e4dd]">{textOrFallback(item.description, "", 160)}</p>
              </div>
            );
          })}
        </div>
